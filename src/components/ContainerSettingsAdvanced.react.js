            ...
            <div className="settings-panel">
                <div className="settings-section">
                    ...
                    <div className="checkboxes">
                        <p><input type="checkbox" checked={this.state.tty} onChange={this.handleChangeTty}/>Allocate a
                            TTY for this container</p>

                        <p><input type="checkbox" checked={this.state.openStdin} onChange={this.handleChangeOpenStdin}/>Keep
                            STDIN open even if not attached</p>

                        <p>Use DNS IP <input id="input-dns-ip" type="text" className="line" placeholder="8.8.8.8"
                                             defaultValue={this.state.dns.ip} onChange={this.handleDnsChange}></input>
                            <span className="text-danger">{this.state.dns.error}</span></p>

                        <p>Use DNS Search<input id="input-dns-search" type="text" className="line"
                                                placeholder="domain.com" defaultValue={this.state.dns.search}
                                                onChange={this.handleDnsSearchChange}></input> <span
                            className="text-danger">{this.state.dns.errorDomain}</span></p>
                    </div>
                    <a className="btn btn-action" disabled={this.props.container.State.Updating}
                       onClick={this.handleSaveAdvancedOptions}>Save</a>
                </div>
            </div>
        );
    }
